...
- There are two forms — the lead-magnet opt-in (`#roadmap-form`) and the consultation enquiry (`#enquiry-form`). Both validate in the browser, then submit via `fetch` to [Formspree](https://formspree.io) (`submitToFormspree()`), which relays them to the site owner's inbox — there's still no first-party backend. Set your real endpoint ID in `FORMSPREE_FORM_ID` before going live.
